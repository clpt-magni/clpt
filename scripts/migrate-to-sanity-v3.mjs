...
const allRecords = [
  { slug: 'nss-unit-1', items: nssUnit1Activities },
  { slug: 'nss-unit-2', items: nssUnit2Activities },
  { slug: 'ipa-lam', items: ipaActivities },
  { slug: 'ispor-amaravati', items: isporAmaravatiActivities },
  { slug: 'ispor-anu', items: isporAnuActivities }
];

async function migrate() {
  console.log("Starting Specialized Collection migration (v3)...");

  for (const record of allRecords) {
    const type = typeMapping[record.slug];
    console.log(`Migrating ${record.items.length} activities for type ${type}...`);
    ...
    for (const item of record.items) {
      ...
      try {
        ...
        console.log(`Pushed ${type}: ${item.name}`);
      } catch (err) {
  ...
  console.log("Specialized Collection migration complete!");
}
