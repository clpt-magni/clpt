...
import { ActivityReport, AttendedActivity } from "@/lib/activity-actions";
import { FacultyMember } from "@/components/faculty/FacultyCard";
import { staticAttendedActivities } from "@/data/attended-activities";
import { 
  Calendar, 
  MapPin, 
  Users, 
...
interface ActivitiesClientProps {
  ...
  initialAttendedActivities?: AttendedActivity[];
...
export default function ActivitiesClient({ 
  initialActivities,
  initialAttendedActivities = [],
  facultyList = []
}: ActivitiesClientProps) {
  const [mainTab, setMainTab] = useState<"organised" | "attended">("organised");
  ...
  const attendedActivities = useMemo(() => {
    if (initialAttendedActivities && initialAttendedActivities.length > 0) {
      return initialAttendedActivities;
    }
    return staticAttendedActivities.map((staticAct) => {
      const matchedFaculty = facultyList
        .filter((f) =>
          staticAct.facultyNames.some((name) =>
      ...
      const resolvedFaculty =
        matchedFaculty.length > 0
          ? matchedFaculty
          : staticAct.facultyNames.map((name) => {
              const prefix = name.startsWith("Dr.")
                ? "Dr."
                ...
                ? "Mrs."
                : undefined;
              const cleanName = prefix ? name.replace(prefix + " ", "") : name;
              return {
                _id: `mock-${name}`,
                name: cleanName,
                prefix,
      ...
      return {
        _id: staticAct._id,
        eventName: staticAct.eventName,
        organizedBy: staticAct.organizedBy,
        date: staticAct.date,
        section: staticAct.section,
        faculty: resolvedFaculty,
      } as AttendedActivity;
    });
  }, [initialAttendedActivities, facultyList]);

  const seminarEvents = useMemo(() => attendedActivities.filter((act) => act.section === "seminar"), [attendedActivities]);
  const wdhEvents = useMemo(() => attendedActivities.filter((act) => act.section === "wdh"), [attendedActivities]);
  const fdpEvents = useMemo(() => attendedActivities.filter((act) => act.section === "fdp"), [attendedActivities]);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="flex bg-slate-200/60 p-1.5 rounded-[1.75rem] max-w-md mx-auto shadow-inner border border-slate-200 mb-6">
        <button
          onClick={() => setMainTab("organised")}
          className={`flex-1 py-3 px-6 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all duration-300 flex items-center justify-center gap-2 ${
            mainTab === "organised"
              ? "bg-slate-900 text-white shadow-md"
              : "text-slate-500 hover:text-slate-800"
          }`}
        >
          <Calendar size={14} /> Organised
        </button>
        <button
          onClick={() => setMainTab("attended")}
          className={`flex-1 py-3 px-6 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all duration-300 flex items-center justify-center gap-2 ${
            mainTab === "attended"
              ? "bg-slate-900 text-white shadow-md"
              : "text-slate-500 hover:text-slate-800"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start animate-in fade-in duration-500">
          ...
          <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden p-6 md:p-8 space-y-6">
            <div className="flex items-center gap-3.5 border-b border-slate-100 pb-5">
              ...
              <div>
                <h3 className="font-poppins font-black text-slate-800 tracking-tight text-base uppercase leading-tight">Seminar/Symposia</h3>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-0.5">{seminarEvents.length} Events Attended</p>
              </div>
            </div>
            
            <div className="space-y-6 divide-y divide-slate-100">
              {seminarEvents.length === 0 ? (
                <p className="text-sm font-semibold text-slate-400 text-center py-6">No seminars registered.</p>
              ) : (
                seminarEvents.map((event, idx) => (
                  <div key={event._id} className={`pt-6 ${idx === 0 ? 'pt-0 border-t-0' : ''} space-y-4`}>
                    <div className="space-y-1.5">
                      <span className="text-[10px] font-black text-slate-400 tracking-widest uppercase">{new Date(event.date).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</span>
                      <h4 className="font-black font-poppins text-slate-800 text-sm leading-snug uppercase">
                        {event.eventName}
                      </h4>
                      <p className="text-xs font-semibold text-slate-500">
                        Organised by: <strong className="text-slate-700 uppercase">{event.organizedBy}</strong>
                      </p>
                    </div>
                    
                    <div className="flex flex-wrap items-center gap-1.5 pt-1">
                      <span className="text-[9px] font-black uppercase text-slate-400 tracking-wider flex items-center gap-1 mr-1 shrink-0"><Users size={12} /> Faculty:</span>
                      {event.faculty?.map((fac) => (
                        fac.slug?.current ? (
                          <Link 
                            key={fac._id} 
                            href={`/faculty/${fac.slug.current}`}
                            className="px-2.5 py-1 bg-primary/5 hover:bg-primary text-primary hover:text-white border border-primary/10 rounded-full text-[10px] font-bold tracking-tight transition-all duration-300"
                          >
                            {fac.prefix ? `${fac.prefix} ` : ''}{fac.name}
                          </Link>
                        ) : (
                          <span 
                            key={fac._id} 
                            className="px-2.5 py-1 bg-slate-100 border border-slate-200/50 rounded-full text-[10px] font-bold text-slate-600 tracking-tight"
                          >
                            {fac.prefix ? `${fac.prefix} ` : ''}{fac.name}
                          </span>
                        )
          ...
          <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden p-6 md:p-8 space-y-6">
            <div className="flex items-center gap-3.5 border-b border-slate-100 pb-5">
              <div className="p-3.5 bg-green-500/10 text-green-600 rounded-2xl">
                <Wrench size={24} />
              </div>
              <div>
                <h3 className="font-poppins font-black text-slate-800 tracking-tight text-base uppercase leading-tight">WDH</h3>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-0.5">{wdhEvents.length} Events Attended</p>
              </div>
            </div>
            
            <div className="space-y-6 divide-y divide-slate-100">
              {wdhEvents.length === 0 ? (
                <p className="text-sm font-semibold text-slate-400 text-center py-6">No workshops registered.</p>
              ) : (
                wdhEvents.map((event, idx) => (
                  <div key={event._id} className={`pt-6 ${idx === 0 ? 'pt-0 border-t-0' : ''} space-y-4`}>
                    <div className="space-y-1.5">
                      <span className="text-[10px] font-black text-slate-400 tracking-widest uppercase">{new Date(event.date).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</span>
                      <h4 className="font-black font-poppins text-slate-800 text-sm leading-snug uppercase">
          ...
          {/* Section 3: FTP/FDP/FEP/STTP */}
          <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden p-6 md:p-8 space-y-6">
            <div className="flex items-center gap-3.5 border-b border-slate-100 pb-5">
              <div className="p-3.5 bg-orange-500/10 text-orange-600 rounded-2xl">
                <Award size={24} />
              </div>
              <div>
                <h3 className="font-poppins font-black text-slate-800 tracking-tight text-base uppercase leading-tight">FTP/FDP/FEP/STTP</h3>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-0.5">{fdpEvents.length} Events Attended</p>
              </div>
            </div>
            
            <div className="space-y-6 divide-y divide-slate-100">
              {fdpEvents.length === 0 ? (
                <p className="text-sm font-semibold text-slate-400 text-center py-6">No development programs registered.</p>
              ) : (
                fdpEvents.map((event, idx) => (
                  <div key={event._id} className={`pt-6 ${idx === 0 ? 'pt-0 border-t-0' : ''} space-y-4`}>
                    <div className="space-y-1.5">
                      <span className="text-[10px] font-black text-slate-400 tracking-widest uppercase">{new Date(event.date).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}</span>
                      <h4 className="font-black font-poppins text-slate-800 text-sm leading-snug uppercase">
