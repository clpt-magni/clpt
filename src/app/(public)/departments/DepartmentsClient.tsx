...
import { Microscope, Activity, FlaskConical, Stethoscope, Pill, Mail, LucideIcon } from "lucide-react";
...
export default function DepartmentsPage({ initialDepartments = [] }: { initialDepartments?: DepartmentItem[] }) {
  const departments = initialDepartments && initialDepartments.length > 0 ? initialDepartments : defaultDepartments;
      ...
      <div className="container mx-auto px-4 py-12">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {departments.map((dept, i) => {
            const IconComponent = typeof dept.icon === "string" 
              ? (ICON_MAP[dept.icon] || FlaskConical) 
              : (dept.icon || FlaskConical);

            return (
              <Card key={dept._id || i} className="border-none shadow-xl bg-white rounded-3xl overflow-hidden group hover:shadow-2xl transition-all transform hover:-translate-y-1">
                <CardContent className="p-10 flex flex-col h-full">
                  <div className="flex items-center gap-6 mb-8 border-b border-slate-100 pb-8">
                    <div className="bg-slate-50 p-6 rounded-2xl group-hover:bg-primary transition-colors">
                      <IconComponent className="text-primary w-10 h-10 group-hover:text-white transition-colors" />
                    </div>
                  ...
                  <Link href={`/faculty?department=${encodeURIComponent(dept.name)}#faculty-directory`}>
